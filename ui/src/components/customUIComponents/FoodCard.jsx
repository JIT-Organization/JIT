...
const FoodCard = ({ food, quantity, handleUpdateQty, openCustomizeDialog, }) => {
  ...
  return (
    <Card className="w-full border-2 border-yellow-400 rounded-xl overflow-hidden shadow hover:shadow-md transition-all relative">
      <CardHeader className="relative p-0" onClick={() => setIsEditable(true)}>
        <div className="relative h-32 w-full">
          ...
          <Badge
            variant="secondary"
            className="absolute top-2 left-2 bg-white text-green-700 text-xs font-medium px-2 py-0.5 rounded-full shadow"
          >
            ⭐ {food.rating || "4.5"}
          </Badge>
          ...
          <Badge
            variant="default"
            className="absolute bottom-2 right-2 bg-black text-white text-[10px] px-2 py-0.5 rounded shadow"
          >
            {food.tag || "New Arrival"}
          </Badge>

          {quantity > 0 && !isEditable && (
            <Badge className="absolute top-2 right-2 bg-green-600 text-white text-[10px] px-2 py-0.5 rounded-full shadow">
              Selected ({quantity})
            </Badge>
          )}
      ...
      >
        <div className="flex justify-between items-center">
          ...
          <div
            ...
            } flex items-center justify-center bg-white rounded-[2px]`}
          >
            <div
              className={`w-2 h-2 ${
