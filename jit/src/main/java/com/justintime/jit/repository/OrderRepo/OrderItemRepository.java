...
public interface OrderItemRepository extends BaseRepository<OrderItem, Long> {
        ...
        List<OrderItem> findByRestaurantCodeAndOrderNumber(@Param("restaurantCode") String restaurantCode,
        ...
        @Query(value = "SELECT oi.* FROM order_item oi " +
                        ...
                        "AND mi.menu_item_name = :itemName", nativeQuery = true)
        OrderItem findByRestaurantCodeAndItemNameAndOrderNumber(@Param("restaurantCode") String restaurantCode,
                        @Param("itemName") String itemName, @Param("orderNumber") String orderNumber);

        void deleteByOrderIdAndMenuItemId(
                        Long orderId,
                        Long menuItemId);

        // @Query("SELECT oi FROM OrderItem oi WHERE (oi.assignedCookId = :cookId) OR
        // (oi.assignedCookId IS NULL AND (oi.order.orderDate IS NULL OR
        // oi.order.orderDate - oi.preparationTime <= :now))")
        // List<OrderItem> findOrderItemsForCook(@Param("cookId") Long cookId,
        ...
        @Query("SELECT oi FROM OrderItem oi " +
                        ...
                        "ORDER BY oi.createdDttm ASC")
        List<OrderItem> findAssignedAndStartedItemsByRestaurantCodeAndCook(
                        @Param("restaurantCode") String restaurantCode, @Param("cookId") Long cookId);
        ...
        List<OrderItem> findByOrder_Restaurant_RestaurantCodeAndOrderItemStatusIn(String restaurantCode,
